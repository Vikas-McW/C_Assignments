...
void pq_insert(struct tnode *p)
{
    ...
    if (curr == qhead)
    { /* Insert before the current start */
        p->next = qhead;
        qhead = p;
    }
    else
...
*/
void generate_code_helper(struct tnode *node, int depth)
{
    if (node->parent != NULL)
    {
        if (node->parent->left == node)
        {
            code[node->symbol][depth] = '0';
        }
        else
        {
            code[node->symbol][depth] = '1';
        }
        generate_code_helper(node->parent, depth + 1);
    }
}
