...
struct token_queue infix_to_postfix(struct token_queue * pqueue_infix) {
    ...
    struct token_queue stack_operators;

    /* initialize the queues to empty */
    queue_postfix.front = NULL;
    queue_postfix.back = NULL;
    stack_operators.front = NULL;
    stack_operators.back = NULL;

    p_expr_token current_token;

    while ((current_token = dequeue(pqueue_infix)) != NULL) {
        switch (current_token->type) {
            case OPERAND:
                enqueue(&queue_postfix, current_token);
                break;
            case OPERATOR:
                while (stack_operators.front != NULL &&
                       (op_precedences[current_token->value.op_code] < op_precedences[stack_operators.front->value.op_code] ||
                        (op_precedences[current_token->value.op_code] == op_precedences[stack_operators.front->value.op_code] &&
                         op_associativity[current_token->value.op_code] == LEFT))) {
                    enqueue(&queue_postfix, pop(&stack_operators.front));
                }
                push(&stack_operators.front, current_token);
                break;
#if PARSE_PARENS
            case LPARENS:
                push(&stack_operators.front, current_token);
                break;
            case RPARENS:
                while (stack_operators.front != NULL && stack_operators.front->type != LPARENS) {
                    enqueue(&queue_postfix, pop(&stack_operators.front));
                }
                if (stack_operators.front != NULL && stack_operators.front->type == LPARENS) {
                    pop(&stack_operators.front); /* Pop the left parenthesis */
                } else {
                    /* Mismatched parentheses - handle error as needed */
                }
                break;
#endif
        }
...
/* postcondition: returned value is final answer, and pqueue_postfix should be empty */
double evaluate_postfix(struct token_queue * pqueue_postfix) {
    struct token_queue stack_operands;
    stack_operands.front = NULL;
    stack_operands.back = NULL;

    while (pqueue_postfix->front != NULL) {
        p_expr_token current_token = dequeue(pqueue_postfix);
        switch (current_token->type) {
            case OPERAND:
                push(&stack_operands.front, current_token);
                break;
            case OPERATOR:
                if (op_operands[current_token->value.op_code] == 1) {
                    /* Unary operator */
                    p_expr_token operand = pop(&stack_operands.front);
                    switch (current_token->value.op_code) {
                        case NEGATE:
                    ...
                    push(&stack_operands.front, operand);
                } else {
                    /* Binary operator */
                    p_expr_token operand2 = pop(&stack_operands.front);
                    p_expr_token operand1 = pop(&stack_operands.front);
                    switch (current_token->value.op_code) {
                        case ADD:
                            operand1->value.operand += operand2->value.operand;
                            break;
                        case SUBTRACT:
                            operand1->value.operand -= operand2->value.operand;
                            break;
                    ...
                    push(&stack_operands.front, operand1);
                    free(operand2);
                }
                break;
        }
    ...
    double result = stack_operands.front->value.operand;
    free(pop(&stack_operands.front)); /* Free the remaining operand */
