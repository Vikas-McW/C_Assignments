...
int compare_first_name(const void *a, const void *b)
{
    ...
    const char *x = student_a->fname;
    const char *y = student_b->fname;
...
int compare_last_name(const void *a, const void *b)
{
    const struct student *student_a = (const struct student *)a;
    const struct student *student_b = (const struct student *)b;
    const char *x = student_a->lname;
    const char *y = student_b->lname;
...
{
    struct student *pstud = (struct student *)prec;
    int *age_limit = (int *)arg;

    if (pstud->age > *age_limit)
    {
        printf("%-20s %-20s %2d %2d\n", pstud->fname, pstud->lname, pstud->year, pstud->age);
    }
}
